//! Stand-alone Linux specific material for the Runtime Manager enclave
//!
//! ## Authors
//!
...
/// Incoming address to listen on (0.0.0.0 implies all addresses)
const ADDR: &'static str = "0.0.0.0:5022";

/// The main function for the Nitro Runtime Manager enclave
pub fn linux_main() -> Result<(), RuntimeManagerError> {
    let listener = TcpListener::bind(ADDR)?;
    // NOTE we are using eprintln here because of terrible interaction with
    // rust test and process io
    // https://github.com/rust-lang/rust/issues/35136
    eprintln!("runtime_manager_linux::linux_main created TcpListener {:?}", ADDR);
    ...
    let (mut fd, client_addr) = listener.accept()?;
    eprintln!("runtime_manager_linux::linux_main connected {:?}", client_addr);

    loop {
        let received_buffer = receive_buffer(&mut fd)?;
        ...
        let return_message = match received_message {
            RuntimeManagerMessage::Initialize(policy_json) => {
                eprintln!("runtime_manager_linux::initialize started");
                managers::session_manager::init_session_manager(&policy_json)?;
                eprintln!("runtime_manager_linux::main init_session_manager completed");
                RuntimeManagerMessage::Status(NitroStatus::Success)
            }
            RuntimeManagerMessage::GetEnclaveCert => {
                eprintln!("runtime_manager_linux::main GetEnclaveCert");
            ...
            RuntimeManagerMessage::GetEnclaveName => {
                eprintln!("runtime_manager_linux::main GetEnclaveName");
            ...
            RuntimeManagerMessage::NewTLSSession => {
                eprintln!("runtime_manager_linux::main NewTLSSession");
            ...
            RuntimeManagerMessage::CloseTLSSession(session_id) => {
                eprintln!("runtime_manager_linux::main CloseTLSSession");
            ...
            RuntimeManagerMessage::GetTLSDataNeeded(session_id) => {
                eprintln!("runtime_manager_linux::main GetTLSDataNeeded");
            ...
            RuntimeManagerMessage::SendTLSData(session_id, tls_data) => {
                eprintln!("runtime_manager_linux::main SendTLSData");
            ...
            RuntimeManagerMessage::GetTLSData(session_id) => {
                eprintln!("runtime_manager_linux::main GetTLSData");
            ...
            RuntimeManagerMessage::GetPSAAttestationToken(challenge) => {
                eprintln!("runtime_manager_linux::main GetPSAAttestationToken");
                // TODO this is a stub!
                todo!()
            }
            RuntimeManagerMessage::ResetEnclave => {
                // Do nothing here for now
                eprintln!("runtime_manager_linux::main ResetEnclave");
                RuntimeManagerMessage::Status(NitroStatus::Success)
            }
            _ => {
                eprintln!("runtime_manager_linux::main Unknown Opcode");
        ...
        let return_buffer = bincode::serialize(&return_message)
            .map_err(|err| RuntimeManagerError::BincodeError(err))?;
        eprintln!(
            "runtime_manager_linux::main calling send buffer with buffer_len:{:?}",
            return_buffer.len()
        );
        send_buffer(&mut fd, &return_buffer)?;
    }
}
...
//    eprintln!("runtime_manager_nitro::nitro_main calling accept");
//
...
//    eprintln!("runtime_manager_nitro::nitro_main accept succeeded. looping");
//    loop {
//        let received_buffer =
...
//            RuntimeManagerMessage::GetEnclaveCert => {
//                eprintln!("runtime_manager_nitro::main GetEnclaveCert");
//                let return_message = match managers::session_manager::get_enclave_cert_pem() {
//                    Ok(cert) => RuntimeManagerMessage::EnclaveCert(cert),
//                    Err(_) => RuntimeManagerMessage::Status(NitroStatus::Fail),
//                };
...
//            }
//            RuntimeManagerMessage::GetEnclaveName => {
//                eprintln!("runtime_manager_nitro::main GetEnclaveName");
//                let return_message = match managers::session_manager::get_enclave_name() {
//                    Ok(name) => RuntimeManagerMessage::EnclaveName(name),
//                    Err(_) => RuntimeManagerMessage::Status(NitroStatus::Fail),
//                };
...
//            }
//            RuntimeManagerMessage::NewTLSSession => {
//                eprintln!("runtime_manager_nitro::main NewTLSSession");
//                let ns_result = managers::session_manager::new_session();
//                let return_message: RuntimeManagerMessage = match ns_result {
//                    Ok(session_id) => RuntimeManagerMessage::TLSSession(session_id),
//                    Err(_) => RuntimeManagerMessage::Status(NitroStatus::Fail),
...
//            }
//            RuntimeManagerMessage::CloseTLSSession(session_id) => {
//                eprintln!("runtime_manager_nitro::main CloseTLSSession");
//                let cs_result = managers::session_manager::close_session(session_id);
//                let return_message: RuntimeManagerMessage = match cs_result {
//                    Ok(_) => RuntimeManagerMessage::Status(NitroStatus::Success),
//                    Err(_) => RuntimeManagerMessage::Status(NitroStatus::Fail),
...
//            }
//            RuntimeManagerMessage::GetTLSDataNeeded(session_id) => {
//                eprintln!("runtime_manager_nitro::main GetTLSDataNeeded");
//                let return_message = match managers::session_manager::get_data_needed(session_id) {
//                    Ok(needed) => RuntimeManagerMessage::TLSDataNeeded(needed),
//                    Err(_) => RuntimeManagerMessage::Status(NitroStatus::Fail),
//                };
...
//            }
//            RuntimeManagerMessage::SendTLSData(session_id, tls_data) => {
//                eprintln!("runtime_manager_nitro::main SendTLSData");
//                let return_message = match managers::session_manager::send_data(session_id, &tls_data)
//                {
//                    Ok(_) => RuntimeManagerMessage::Status(NitroStatus::Success),
//                    Err(_) => RuntimeManagerMessage::Status(NitroStatus::Fail),
...
//            }
//            RuntimeManagerMessage::GetTLSData(session_id) => {
//                eprintln!("runtime_manager_nitro::main GetTLSData");
//                let return_message = match managers::session_manager::get_data(session_id) {
//                    Ok((active, output_data)) => RuntimeManagerMessage::TLSData(output_data, active),
//                    Err(_) => RuntimeManagerMessage::Status(NitroStatus::Fail),
//                };
...
//            }
//            RuntimeManagerMessage::GetPSAAttestationToken(challenge) => {
//                eprintln!("runtime_manager_nitro::main GetPSAAttestationToken");
//                get_psa_attestation_token(&challenge)?
//            }
//            RuntimeManagerMessage::ResetEnclave => {
//                // Do nothing here for now
//                eprintln!("runtime_manager_nitro::main ResetEnclave");
//                RuntimeManagerMessage::Status(NitroStatus::Success)
//            }
//            _ => {
//                eprintln!("runtime_manager_nitro::main Unknown Opcode");
//                RuntimeManagerMessage::Status(NitroStatus::Unimplemented)
//            }
//        };
//        let return_buffer = bincode::serialize(&return_message)
...
//    eprintln!("runtime_manager_nitro::initialize started");
//    managers::session_manager::init_session_manager(policy_json)?;
//    eprintln!("runtime_manager_nitro::main init_session_manager completed");
...
//    eprintln!("runtime_manager_nitro::get_psa_attestation_token started");
//    eprintln!(
